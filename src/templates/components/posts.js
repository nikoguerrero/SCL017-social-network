import { createPostTemplate } from '../composePost.js';
    ...
    if (textDescription.value.length == '') {
      ...
      if (editPostId === null){ // si no hay post a editar, agrega un nuevo post
        ...
        await db.collection('post').doc(editPostId).set({
          textDescription: createPostTemplate(textDescription.value)
        });
      editPostId = null;
    }
  }
  ...
  db.collection('post').onSnapshot(snapshot => {
    ...
    changes.forEach(change => {
      ...
      if (change.type === 'added') {
        ...
        postsList.querySelector('#postedTextId').textContent = change.doc.data().createPostTemplate(textDescription); // reescribir de forma inmediata el texte area. 
...
const editUserPost = () => {
  ...
  edit.addEventListener('click', async (e) => {
    ...
    createPostTemplate(textDescription.value) = postData.data().createPostTemplate(textDescription);
  });
};
